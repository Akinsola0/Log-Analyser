...
No environment variables are needed for the mock data. Copy `.env.example` to `.env.local` and fill in the Supabase URL and anon key when the backend team provisions the project — until then the auth screens explain that instead of rendering a broken form.
